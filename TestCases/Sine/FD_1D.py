...
def main(p,CFL,Tfin,c,D,init,grad_init,bcond,Yl,Yr,N,L,timeIntegration="RK6low",cellmask="Regular",ksin=np.pi/2):
...
#Initial conditions
    ...
    for i in range(len(x)):
        ...
        elif init=='Constant': # u0 = gaussienne 
            sol[i]=0.
...
#Used for comp.py
    ...
    for itime in range(niter+1):
        ...
        for ik in range(len(alpha)):
            ...
            if(bcond==1):
                if(c>0.):
                    ...
                    dflux_it_cont = c*(np.roll(sol,-1)-sol)/dx[0]
            elif(bcond==0):
        ...
        l2[itime+1] = simps(sol[int(0.8*(p+1)*N):int(0.9*(p+1)*N)]-sol[0:int(0.1*(p+1)*N)],x[0:int(0.1*(p+1)*N)])
