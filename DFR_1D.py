...
#Mesh creation with gauss points on isoparametric cells
    ...
    # USELESS FOR DFR (RP)
##Initialisation des points flux
#    
...
#            if init==0:
#                sol[i,j]=m.exp(-(solPointMesh[i,j]+0.10)**2/0.0001) #Calcul des points flux. USELESS FOR DFR
#            elif init==1: 
#                if method==0:
...
# Extrapolation of solution on fluxes [= solutions (RP)] points
                sol_flux_it[icell,:]=np.dot(Extrap,sol_it[icell,:])
                
    # Pourquoi à l'interieur de la boucle ? (RP)
#                if bcondL==0:
...
################################################################################################################### Version DFR ci-dessous (RP)
        ...
        # Extrapolation of solution on borders (RP)
                ...
                sol_flux_it[icell,0] = 0.5*(sol_flux_it_tmp[icell,0]+sol_flux_it_tmp[icell-1,-1]) 
                sol_flux_it[icell,-1]= 0.5*(sol_flux_it_tmp[icell+1,0]+ sol_flux_it_tmp[icell,-1])
                        
        # Lagrangian interpolation of the previous solution (RP)                
                sol_flux_p2[icell,:]=np.dot(Extrap2,sol_flux_it[icell,:])
                sol_flux_p2[icell,:]= sol_flux_it[icell,:,0]*LagrangePoly0 + sol_flux_p2[icell,:] + sol_flux_it[k,-1]*LagrangePolyP+2  TODO (RP)
